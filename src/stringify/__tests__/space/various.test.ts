...
describe("space handling", () => {
  ...
  test.each([
    ...
    expect(stringifyToString(val, null, "")).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, 0)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, "  ")).toBe(JSON.stringify(val, null, "  "));
    expect(stringifyToString(val, null, 2)).toBe(JSON.stringify(val, null, "  "));
    expect(stringifyToString(val, null, 2.5)).toBe(JSON.stringify(val, null, "  "));
    expect(stringifyToString(val, null, -0.5)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, -1)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, -1.5)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, "\t")).toBe(JSON.stringify(val, null, "\t"));
    expect(stringifyToString(val, null, NaN)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, +Infinity)).toBe(JSON.stringify(val, null, " ".repeat(10)));
    expect(stringifyToString(val, null, -Infinity)).toBe(JSON.stringify(val, null, ""));
    expect(stringifyToString(val, null, " ".repeat(11))).toBe(JSON.stringify(val, null, " ".repeat(10)));
  });
  ...
  describe("array with multi-property object", async () => {
    ...
    test.each([
      ...
      expect(stringifyToString(val, null, space)).toBeOneOf(oneOf);
    });
  });
});
